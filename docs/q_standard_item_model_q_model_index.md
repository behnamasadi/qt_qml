...
```cpp
    ...
    QObject::connect(&dataModel,&QStandardItemModel::itemChanged,[=](QStandardItem *item)
    {
        qDebug()<<"item on row: "<<item->row()<<"," <<"col: "<<item->column() << " has changed";
    ...
    QStandardItem *item1 = new QStandardItem(QString("row %0, column %1").arg(4).arg(0));
    QStandardItem *item2 = new QStandardItem(QString("row %0, column %1").arg(4).arg(1));
